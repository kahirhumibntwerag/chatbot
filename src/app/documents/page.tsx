  ...
  function addSubCollection(parentId: string, name: string) {
    setCollections(prev => {
      const newCollections = prev.map(c => {
        if (c.id === parentId) {
          return { ...c, children: [...(c.children || []), { id: `sub-${Date.now()}`, name, count: 0 }] };
        }
        return c;
      });
    ...
    setExpandedIds(prev => {
      const s = new Set(prev);
      s.add(parentId); // ensure parent opens so the new child is visible
      return s;
    });
  }
  ...
  async function handleAddDOI() {
    ...
    try {
      ...
      const newItem: ItemRow = {
        ...
        tags: [],
        collections: ["library", ...(selectedCollectionId && selectedCollectionId !== "library" ? [selectedCollectionId] : [])],
              ...
              <div className="p-3">
                {table.getSelectedRowModel().rows.length > 0 && (
                  <div className="px-3 py-2 text-sm">
                    ...
                    <Button size="sm" className="ml-2">Add Tag</Button>
                  </div>
                )}
